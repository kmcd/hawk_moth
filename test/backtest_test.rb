  ...
  test "should select greatest entry signal when multiple signals present" do
    @backtest.spreads = oh( 
      "2011-07-08 11:09:00".dt => [
        [[:gld, 133.57 ], [:iau, 134.07  ],  -2.6946 ],
        [[:spy, 133.57 ], [:ivv, 134.07  ],  -2.6945 ]
      ],
      "2011-07-08 11:11:00".dt => [
        [[:gld, 133.56],  [:iau, 134.01  ],  1.5032 ],
        [[:spy, 133.56],  [:ivv, 134.01  ],  1.5031 ]
      ])
    
    assert_equal "2011-07-08 11:09:00 11:11:00 long:gld short:iau 11.03", @backtest.trades
  end
end